...
const initialItems: CanvasItem[] = [
  ...
  { id: '2', type: 'task',  x: 340, y: 80,  width: 220, height: 80,  content: 'Diseñar prototipo UI', status: 'done',       color: '#FFFFFF' },
  { id: '3', type: 'task',  x: 340, y: 180, width: 220, height: 80,  content: 'Implementar drag and drop', status: 'inprogress', color: '#FFFFFF' },
  { id: '4', type: 'task',  x: 340, y: 280, width: 220, height: 80,  content: 'Conectar API de clima', status: 'pending',    color: '#FFFFFF' },
...
export function Whiteboard() {
  ...
  const startDraw = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    ctx.strokeStyle = tool === 'eraser' ? '#F6F4FB' : penColor;
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (tool === 'note') {
      const newItem: CanvasItem = {
        ...
        content: 'Nueva nota', color: noteColors[Math.floor(Math.random() * noteColors.length)],
      };
    ...
    if (tool === 'task') {
      const newItem: CanvasItem = {
        ...
        x, y, width: 220, height: 80,
        content: 'Nueva tarea', status: 'pending', color: '#FFFFFF',
      };
      setItems(prev => [...prev, newItem]);
      setTool('select');
      return;
  ...
  const cycleStatus = (id: string) => {
    const cycle = ['pending', 'inprogress', 'done'];
    setItems(prev => prev.map(i => {
      if (i.id !== id || i.type !== 'task') return i;
      const current = cycle.indexOf(i.status || 'pending');
      return { ...i, status: cycle[(current + 1) % 3] as CanvasItem['status'] };
    }));
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: '#F6F4FB' }}>

      {/* Toolbar */}
      <div style={{
        display: 'flex', alignItems: 'center', gap: '8px',
        padding: '10px 20px', backgroundColor: '#FFFFFF',
        borderBottom: '0.5px solid #E4DCF4', flexShrink: 0,
        flexWrap: 'wrap',
      }}>
        <span style={{ fontSize: '0.875rem', fontWeight: 400, color: '#8070C8', marginRight: '8px' }}>Tablero</span>
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          {tools.map(t => (
            <button key={t.key} onClick={() => setTool(t.key)} title={t.label} style={{
              ...
              backgroundColor: tool === t.key ? '#E0D8F8' : 'transparent',
              color: tool === t.key ? '#8070C8' : '#9080B0',
        ...
        <div style={{ width: '0.5px', height: '24px', backgroundColor: '#E4DCF4', margin: '0 4px' }} />
          ...
          <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
            {penColors.map(c => (
              <div key={c} onClick={() => setPenColor(c)} style={{
                ...
                cursor: 'pointer', border: penColor === c ? '2px solid #2F2840' : '2px solid transparent',
                transition: 'border 0.1s',
              }} />
            ))}
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: '4px' }}>
              <span style={{ fontSize: '11px', color: '#B0A0C0' }}>Grosor</span>
              {[2, 4, 8].map(s => (
                <div key={s} onClick={() => setPenSize(s)} style={{
                  width: `${s + 10}px`, height: `${s + 10}px`, borderRadius: '50%',
                  backgroundColor: penSize === s ? '#8070C8' : '#D8D0EC',
                  cursor: 'pointer',
                }} />
              ))}
            </div>
        ...
        {/* Instructions */}
        <span style={{ marginLeft: 'auto', fontSize: '0.8rem', color: '#C0B0D0', fontWeight: 300 }}>
      ...
      <div
        ref={containerRef}
        style={{ flex: 1, position: 'relative', overflow: 'hidden', cursor: tool === 'pen' || tool === 'eraser' ? 'crosshair' : tool === 'select' ? 'default' : 'crosshair' }}
        ...
        <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none' }}>
          <defs>
            <pattern id="grid" width="32" height="32" patternUnits="userSpaceOnUse">
              <circle cx="1" cy="1" r="1" fill="#D8D0EC" opacity="0.4" />
            </pattern>
          </defs>
          ...
          >
            ...
            {item.type === 'note' && (
              <div style={{
                ...
                border: selectedId === item.id ? '1.5px solid #8070C8' : '0.5px solid #E4DCF4',
                borderRadius: '12px', padding: '12px',
                minHeight: item.height, boxShadow: '0 2px 8px rgba(128,112,200,0.1)',
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                  <span style={{ fontSize: '10px', color: '#B0A0C0', fontWeight: 300 }}>nota</span>
                  {selectedId === item.id && (
                    <button onClick={() => deleteItem(item.id)} style={{
                      background: 'none', border: 'none', cursor: 'pointer',
                      fontSize: '12px', color: '#C070A0', padding: 0,
                    }}>✕</button>
                  )}
                </div>
                {editingId === item.id ? (
            ...
            {item.type === 'task' && (
              <div style={{
                ...
                minHeight: item.height, boxShadow: '0 2px 8px rgba(128,112,200,0.08)',
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                  {editingId === item.id ? (
                    <input
                      ...
                      style={{
                        ...
                        fontSize: '0.875rem', color: '#2F2840', fontWeight: 400,
                  ...
                  {selectedId === item.id && (
                    <button onClick={() => deleteItem(item.id)} style={{
                      background: 'none', border: 'none', cursor: 'pointer',
                      fontSize: '12px', color: '#C070A0', padding: 0, flexShrink: 0,
                    }}>✕</button>
                  )}
                </div>
                <button onClick={e => { e.stopPropagation(); cycleStatus(item.id); }} style={{
                  ...
                  backgroundColor: statusStyle[item.status || 'pending'].bg,
                  color: statusStyle[item.status || 'pending'].color,
                }}>
                  {statusStyle[item.status || 'pending'].label}
                </button>
              </div>
            )}
            ...
            {item.type === 'text' && (
              <div style={{
                border: selectedId === item.id ? '1px dashed #C8C0E0' : '1px dashed transparent',
                ...
                {editingId === item.id ? (
                  <input
                    ...
                    style={{
                      ...
                      fontSize: '1rem', color: '#2F2840', fontWeight: 300,
                      outline: 'none', fontFamily: 'inherit', width: '100%',
                    }}
                  />
                ) : (
                  <p onDoubleClick={e => startEdit(e, item)} style={{
                    fontSize: '1rem', color: '#2F2840', fontWeight: 300, margin: 0,
                  }}>{item.content}</p>
                )}
                {selectedId === item.id && (
                  <button onClick={() => deleteItem(item.id)} style={{
